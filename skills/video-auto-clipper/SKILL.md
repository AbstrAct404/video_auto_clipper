...
- 「镜头太碎/不要小于1秒的镜头」「相邻片段要有关联/钩子」→ **已验证质量标准**（节奏守卫与钩子链已内置，按该章节口径回复）
...
2. `segments` 是混剪后的成片时间轴，每段保持原片内完整、不重复使用镜头；
   输出已经过节奏守卫：单段 ≥1s，孤立短镜并入相邻段或剔除，快闪串保留；
3. `notes` 会标注降级/近似情况（如 contrast_open 无对比对回退 peak_first）；
4. 已知局限：恋爱风格（romance_intimacy）本地像素信号常无目标命中，返回
   `no target matched; nothing to plan` 属预期，亲密动作语义需 L2 确认；
   此时如实告知用户并建议改用通用模板或 linear 顺剪，不要当作故障；
5. 人物/行动等语义由本地时间段近似，正式交错需 L2（`prompts/narrative_board.md`）确认，回复用户时需说明。

## 已验证质量标准（实测好评期复盘）

以「这爆发力直接封神！」_2/_3（ZEUS 群像、家庭轻喜剧，用户确认质量最好）
为基准，好评成片的共同结构是：

1. **锚点均匀铺满全片**：15s 成片 = 5 段 × 3s 高分锚点，锚点散布在
   0~130s 的不同区间（间隔 10~40s），覆盖开头/中段/结尾；对照反面案例
   （_1）把 5 段全部挤在前 8s 的高分密集区，观感是重复堆砌。若 L3 选窗
   结果高度聚集，优先改用叙事计划（模板重排天然拉开跨度）或调整
   `motion_window_count` 重跑，而不是直接交付聚集版本。
2. **节奏统一、单镜不碎**：每段 3s 左右的完整镜头段落，段长一致形成
   稳定节奏。系统已内置节奏守卫（`configs/narrative_targets.yaml` 的
   `rhythm` 块 + L3 尾段保护）：
   - 单镜头/片段最短 1.0s；
   - **禁止**两个中长镜头之间夹一个 <1s 的孤立短镜——自动并入相邻原片
     片段，无法并入则剔除；
   - **允许**连续 ≥2 个短片段构成的快闪串（原片自带快闪或模型生成的
     快闪段落），这是有效节奏手法，不视为碎片化。
3. **相邻片段语义关联成钩子**：叙事计划会为相邻片段建立 hook 链接
   （按原片时间邻近 + 共享画面目标 + 运动强度相似三维关联度排序衔接），
   成片不是按分数机械拼接。回复用户解读成片时，应说明相邻镜头间的
   关联逻辑（如「闪电特效→群像集结」的同目标递进），而非只报时间码。

chatbot 使用约定：

1. 生成后自查 segments：任何段 <1s（快闪串除外）都属于异常，应重跑或
   调整 `target_duration_seconds`/窗口数后再交付；
2. 用户反馈「太碎」「跳戏」时，优先检查是否命中孤立短镜或锚点聚集，
   而不是直接换模板。
